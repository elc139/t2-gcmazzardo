/*
  Versão utilizando OPENMP do programa exemplo pthreads_dotprod, utilizado para o Trabalho 2 da disciplina de Programação Paralela
 
  Nome: Gabriel Cortez Mazzardo
  E-mail: gcmazzardo@inf.ufsm.br 
...
{
   int i, k;
   long offset = (long) omp_get_thread_num(); //offset = id da thread
   double *a = dotdata.a;
   double *b = dotdata.b;     
...
int main(int argc, char **argv)
{
   ...
   //seta o num de threads que serao utilizadas na execução do OpenMP
   omp_set_num_threads(nthreads);
   
   #pragma omp parallel //iniciando regiao paralela
   {
